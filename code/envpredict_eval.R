...
layout(matrix(c(1,1,2,3,4,4,5,6), 2, 4, byrow = F))
results_folders = c() 
results_folders[1] = "~/aquatic/envpredict/output/physchem_based_on_seqdata_or_phytoplan"
results_folders[2] = "~/aquatic/envpredict/output/physchem_based_on_seqdata_or_zooplan"
for (res_fol in 1:2) {
  ...
  barplot(t(cor_matr[,c(1,3)]), beside = T, horiz = T, las = 1, legend = T, names.arg = sample_labels, cex.names = 0.8, args.legend = list(x = "top", inset = c(-0.2,0)), xlab = "Pearson cor. (R)")
  par(mar = c(6,2,2,2))
  
  #boxplot(cor_matr[,1], cor_matr[,2], cor_matr[,3],  names = colnames(cor_matr), las = 2, ylim = c(0.2, 1))
  boxplot(cor_matr[,1], cor_matr[,3],  names = colnames(cor_matr)[c(1,3)], las = 2, ylim = c(0.2, 1))
  par(mar = c(4,2,2,2))
  plot(cor_matr[,1], cor_matr[,3], xlim = c(0,1), ylim = c(0,1), xlab = colnames(cor_matr)[1], ylab = colnames(cor_matr)[3])
  lines(c(0,1), c(0,1))
  wilcox.test(cor_matr[,1], cor_matr[,2], paired = T)
  wilcox.test(cor_matr[,1], cor_matr[,3], paired = T)
}
