...
if (argv$b != "") { # processing abiotic data
  ...
  #ab_factors=names(abiotics)[c(11,16,17,18,22)] # changed should be 9:25
  ab_factors=names(abiotics)[c(11)] # changed should be 9:25
  add_sufix="Abiotic"
}
...
for (rRNA in c("16S")) { #rRNA="16S"
  ...
  if (argv$a != "") {
    ...
    for (archt in archt_numbers ) {
      ...
      num=grep(paste("^", archt, "_norm_seqtab", sep =""),VAE_features_files)
      cat("archt to read: ", archt, "\n")
      cat("num to read: ", num, "\n")
      cat("file to read: ", paste(argv$a,VAE_features_files[num], "\n", sep="/"))
